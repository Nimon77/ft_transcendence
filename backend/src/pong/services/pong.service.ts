    ...
    update(room : Room)
    {
        ...
        if (next.x - room.options.ball.radius < 0 || next.x + room.options.ball.radius > room.options.display.width)
        {
            ...
            if (next.x + room.options.ball.radius > room.options.display.width)
                ajust = Math.PI;
            return this.updateBall(room.options.display.width / 2, room.options.display.height / 2, (Math.random() * Math.PI) / 2 - Math.PI / 4 + ajust, room);
        }
        // player 1
        if (next.y >= room.players[0].tray - room.options.tray.height / 2 && next.y <= room.players[0].tray + room.options.tray.height / 2)
        ...
        //player 2
        if (next.y >= room.players[1].tray - room.options.tray.height / 2 && next.y <= room.players[1].tray + room.options.tray.height / 2)
            if (next.x + room.options.ball.radius > room.options.display.width - room.options.tray.x)
              return this.updateBall(room.ball.x, room.ball.y, (Math.random() * Math.PI) / 2 - Math.PI / 4 + Math.PI, room);
        //floor n top
        if (next.y - room.options.ball.radius < 0 || next.y + room.options.ball.radius >room.options.display.height)
